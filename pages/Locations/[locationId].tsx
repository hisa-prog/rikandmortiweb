...
export default function Location({ locationId, residents, location }: any) {
  ...
  return (
    <PageOfEpisode>
      ...
      <Wrapper>
        <Button onClick={() => goToPageLocations()} style={{ height: "55px" }}>
...
export async function getServerSideProps(ctx: any) {
  ...
  const location = (
    await axios.get(`https://rickandmortyapi.com/api/location/${locationId}`)
  ).data;
  ...
  for (let i = 0; i < location.residents.length; i++) {
    ...
    residents =
      i === location.residents.length - 1
        ? residents + tmp
        : residents + tmp + ",";
  }
  residents = (
    await axios.get(`https://rickandmortyapi.com/api/character/${residents}`)
  ).data;
  residents =
    location.residents.length === 1 ? [{ ...(residents as any) }] : residents;
  residents =
    location.residents.length === 0 ? [{ name: "unknown" }] : residents;
