      ...
      <ShowInformationAboutCharacter>
        <ShowImageAndOtherAboutCharacter>
          ...
          <InfoAboutCharacter style={{ fontSize: "35px", paddingTop: "20px" }}>
            ...
            <OriginLocationCharacterOnPage
              onClick={() => TakeLocationId(character.origin.url)}
              style={{ fontSize: "25px", marginLeft: "37px", marginTop: "8px" }}
            >
              Origin location - {character.origin.name}
            </OriginLocationCharacterOnPage>
            <LastLocationCharacterOnPage
              onClick={() => TakeLocationId(character.location.url)}
            ...
            <CharacterBlock
              style={{
                ...
                fontSize: "25px",
                visibility:
                  character.gender === "unknown" ? "hidden" : "visible",
              }}
            >
...
export async function getServerSideProps(ctx: any) {
  ...
  episodes = (
    await axios.get(`https://rickandmortyapi.com/api/episode/${episodes}`)
  ).data;
  episodes =
    character.episode.length === 1 ? [{ ...(episodes as any) }] : episodes;
